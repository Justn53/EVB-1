...
Public Sub InitMessages()
    ...
    HandleDataSub(SAllEditors) = GetAddress(AddressOf HandleAllEditors)
...
Dim Buffer As clsBuffer
    ...
    For x = 1 To MAX_QUESTS
        For j = 1 To MAX_QUEST_OBJECTIVES
            Call SetPlayerDataAmountLeft(i, x, Buffer.ReadLong, j)
        Next
...
Dim x As Long, y As Long, Sprite As Long, i As Long
    ...
    If BloodIndex >= MAX_BLOOD Then BloodIndex = 1
...
Dim i As Byte
    ...
    With frmMain
        ...
        .lblConvMessage.Caption = Trim$(Conv(ConvNum).Chat(chatNum).Text)
        For i = 1 To 4
            .lblConvResp(i).Caption = Trim$(Conv(ConvNum).Chat(chatNum).ReplyText(i))
        Next
    ...
    ' play the sound
    If Trim$(Conv(ConvIndex).Chat(chatNum).Sound) <> vbNullString Then PlaySound Conv(ConvIndex).Chat(chatNum).Sound
End Sub
...
Private Sub HandleAllEditors()
    ...
    Exit Sub
errorhandler:
    HandleError "HandleAllEditors", "modHandleData", Err.Number, Err.Description, Err.Source, Err.HelpContext
    Err.Clear
    Exit Sub
End Sub

Private Sub HandleQuestFinish(ByVal Index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
Dim Buffer As clsBuffer, QuestNum As Long, i As Long
    ...
    ' Drop it
    For i = 1 To 3
        Player(MyIndex).Quest(QuestNum).DataAmountLeft(i) = 0
    Next
    ...
    Exit Sub
errorhandler:
    HandleError "HandleQuestFinish", "modHandleData", Err.Number, Err.Description, Err.Source, Err.HelpContext
    Err.Clear
    Exit Sub
End Sub
