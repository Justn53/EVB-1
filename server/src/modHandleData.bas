...
Public Sub InitMessages()
    ...
    HandleDataSub(CRequestAllEditors) = GetAddress(AddressOf HandleRequestAllEditors)
End Sub
...
Private Sub HandleLogin(ByVal index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
    ...
    If Not IsPlaying(index) Then
        If Not IsLoggedIn(index) Then
            ...
            ' Check versions
            If Buffer.ReadLong < VERSION_MAJOR Or Buffer.ReadLong < VERSION_MINOR Or Buffer.ReadLong < VERSION_REVISION Then
...
Private Sub HandleBanDestroy(ByVal index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
    ...
    If Not FileExist(App.Path & "\data\banlist.txt") Then
        F = FreeFile
                ...
                If Player(index).Inv(i).Num > 0 Then
                    If Player(index).Inv(i).Num = Player(index).Hotbar(Slot).Slot Then
                        If Item(Player(index).Inv(i).Num).Type = ITEM_TYPE_CONSUME Then
                            ' Make sure they don't have others left
                            If DuplicatesInInv(index, Player(index).Inv(i).Num) = 0 Then
...
Dim EmptyReplies As Byte
    ...
    Select Case Conv(ConvIndex).Chat(CurChat).Event
        ...
        Case 8 ' finish quest
            If Quest(Conv(ConvIndex).Chat(CurChat).Data1).Objective(1).ObjectiveType = 1 And TempPlayer(index).InChat = Quest(Conv(ConvIndex).Chat(CurChat).Data1).Objective(1).DataIndex Then
                Call FinishQuest(index, Conv(ConvIndex).Chat(CurChat).Data1)
            Else
                Call CheckFinishQuest(index, Conv(ConvIndex).Chat(CurChat).Data1)
            End If
        Case Else
...
Dim QuestNum As Long, j As Long
    ...
    For j = 1 To MAX_QUEST_OBJECTIVES
        Player(index).Quest(QuestNum).DataAmountLeft(j) = 0
    Next
...
Private Sub HandleRequestAllEditors(ByVal index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
Dim Buffer As clsBuffer
